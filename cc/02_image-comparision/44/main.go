	...
	for _, needle := range images {
		for _, haystack := range images {
			...
			for k, v := range haystack.pixels {
				var diff int64
				diff += int64(math.Abs(float64(v.r - needle.pixels[0].r)))
				diff += int64(math.Abs(float64(v.g - needle.pixels[0].g)))
				diff += int64(math.Abs(float64(v.b - needle.pixels[0].b)))
				diff += int64(math.Abs(float64(v.a - needle.pixels[0].a)))
				if diff < 10 {
					x := k % haystack.width
					y := k / haystack.width
					fmt.Println("PIXEL", k, x, y, "DIFF", diff)
				}
			}
