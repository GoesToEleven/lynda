...
func comparePixels(n, h *image, ch chan result) {
	...
	for i := 0; i < (h.width * h.height); i++ {
		...
		// reverse compare
		// the only change: if statement: x > h.width - n.width
...
func compareSequenceReverse(n, h *image, hIdx int, ch chan result) {
	...
	for i := 0; i < n.height*n.width; i++ {
		...
		//debugging
		if (i % n.width == 0) && ((i / n.width) == 0) {
			fmt.Println("reverse compare: ", n.name, h.name, hIdx,
				hIdx/h.width, hIdx%h.width, "\n",
				n.pixels[0].r, n.pixels[0].g, n.pixels[0].b, n.pixels[0].a, "\n",
				h.pixels[hIdx].r, h.pixels[hIdx].g, h.pixels[hIdx].b, h.pixels[hIdx].a, "\n")
		}

		// if start of new row, align pixels
		if (i % n.width == 0) && ((i / n.width) != 0) {
			hIdx += (n.width + h.width) // this changed
		}
